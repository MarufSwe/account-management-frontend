<template>
  <div v-if="all_report_sheet">
    <router-link :to="{path: 'home'}">
      <h3 style="font-weight: bold" class="text-primary">Report Sheet</h3>
    </router-link>
    <table class="table table-hover">
      ...
      <tr
        class="tableRow"
        v-for="(report, SL) in all_report_sheet"
        :key="report">
        <td>{{SL+1}}</td>
        <td>{{report.brand}}</td>
        ...
        <td class="td_action">
          <i
            ...
            @click="deleteEducationItem(report.id) in all_report_sheet"
          ></i>
        </td>
      </tr>
    </table>
  ...
  methods: {
    GetReportList: function () {
      axios.get('http://127.0.0.1:8000/api/create-report/').then((response) => {
        this.all_report_sheet = response.data.results
        console.log('ALLDATA##', this.all_report_sheet)
